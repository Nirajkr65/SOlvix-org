...
const ProblemEditor = () => {
    ...
    const [leftWidth, setLeftWidth] = useState(45); // Initial width 45%
    ...
    const TabButton = ({ active, onClick, icon: Icon, label }) => (
        <button
            onClick={onClick}
            className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium transition-all relative border-b-2 z-10 whitespace-nowrap shrink-0 
            ${active
    ...
    return (
        <div className="h-[calc(100vh-4rem)] flex flex-col bg-black text-white selection:bg-emerald-500/30 overflow-hidden relative">
            {/* Background Noise */}
            <div className="absolute inset-0 z-0 pointer-events-none opacity-20 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] brightness-100 contrast-150"></div>

            {/* Top Navigation Bar */}
            <div className="h-14 border-b border-white/10 bg-black/40 backdrop-blur-md flex items-center px-4 justify-between shrink-0 z-20 relative">
            ...
            <div className="flex-1 flex overflow-hidden">
                {/* LEFT PANEL */}
                <div style={{ width: `${leftWidth}%` }} className="flex flex-col border-r border-white/10 bg-black/20 backdrop-blur-sm relative z-10 shrink-0">
                    {/* Left Tabs */}
                    <div className="flex items-center border-b border-white/10 bg-black/20 overflow-x-auto no-scrollbar">
                        ...
                        <TabButton active={activeLeftTab === "chatAI"} onClick={() => setActiveLeftTab("chatAI")} icon={Zap} label="AI Help" />
                ...
                {/* RESIZER SPLITTER */}
                <div
                    className={`w-2 bg-black hover:bg-emerald-500/50 cursor-col-resize flex items-center justify-center transition-colors z-20 shrink-0 relative group ${isResizing ? 'bg-emerald-500/50' : 'border-l border-r border-white/5'}`}
                    onMouseDown={startResizing}
                >
                    <div className="absolute inset-y-0 -left-1 -right-1 z-30" /> {/* Hit area */}
                    <GripVertical className={`w-4 h-4 text-zinc-600 transition-colors group-hover:text-white ${isResizing ? 'text-white' : ''}`} />
                </div>

                {/* RIGHT PANEL (Editor & Results) */}
                <div style={{ width: `${100 - leftWidth}%` }} className="flex flex-col bg-[#1e1e1e] shrink-0">
