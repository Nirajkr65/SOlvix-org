...
const Editorial = ({ problemId, secureURL, thumbnailURL, duration, referenceSolution }) => {
    ...
    return (
        <div className="space-y-8">
            ...
            {editorialContent ? (
                ...
                !loadingContent && !secureURL && !referenceSolution && (
            ...
            {/* 3. Reference Solution (Fallback or Additional) */}
            {(!editorialContent || !editorialContent.code || editorialContent.code.length === 0) && referenceSolution && referenceSolution.length > 0 && (
                <div className="animate-fade-in space-y-6 pt-4 border-t border-white/10">
                    ...
                    <div className="space-y-6">
                        {referenceSolution.map((sol, index) => (
                            <div key={index} className="border border-white/10 rounded-xl overflow-hidden">
                                <div className="bg-[#252526] px-4 py-2 border-b border-white/10 text-xs font-bold text-zinc-300 uppercase flex justify-between items-center">
                                    <span>{sol.language || `Solution ${index + 1}`}</span>
                                </div>
                                <div className="h-[400px]">
                                    <Editor
                                        height="100%"
                                        language={sol.language === "c++" || sol.language === "cpp" ? "cpp" : sol.language || "javascript"}
                                        value={sol.completecode || sol.completeCode || "// Code not found"}
                                        theme="vs-dark"
                                        options={{
                                            readOnly: true,
                                            minimap: { enabled: false },
